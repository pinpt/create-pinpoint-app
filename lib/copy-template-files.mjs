...
export const copyAndFilterFiles = async ({ options }) => {
	const { id: siteId, slug } = await selectSite(options.host);
		...
		const outdir = await prompts(
			{
				...
				validate: (value) => fs.existsSync(path.dirname(resolvePath(value))),
				initial: dir,
			},
			{
				onCancel: () => {
	...
	try {
		...
		// make the name of the template the slug
		const pfn = path.join(dir, 'package.json');
		const buf = JSON.parse(fs.readFileSync(pfn).toString());
		buf.name = config.slug;
		fs.writeFileSync(pfn, JSON.stringify(buf, null, 2));
